#!/usr/local/uvcdat/1.3.1/Library/Frameworks/Python.framework/Versions/2.7/Resources/Python.app/Contents/MacOS/Python
                ...
                if vartime is not None:
                    try:
                        ...
                        if timeIsLinear is None and timeIsVector is None:
                            print " referenceTime = ", str(referenceTime)
...
def test_points( data_file, grid_file, varname, iTile ): 
...
#    element_corners = gf['element_corners']
    lat_data = lat[:].raw_data()
    lon_data = lon[:].raw_data()
#    print str( dir( lon_data ) )
#    print str( lon_data.__class__ )
    z_data = numpy.zeros_like( lat_data )
    points_data = numpy.dstack( ( lon_data, lat_data, z_data ) ).flatten()
    
    vtk_points_data = vtk.vtkDoubleArray() 
    vtk_points_data.SetNumberOfTuples( lon_data.size )
    vtk_points_data.SetNumberOfComponents( 3 )
    vtk_points_data.SetVoidArray( points_data, points_data.size, 1 )    
    vtk_points = vtk.vtkPoints()
    vtk_points.SetData( vtk_points_data )
    
    npts_tot = lat.shape[0]
    npts_tile = npts_tot / nTiles
    dim = int( math.sqrt( npts_tile ) )
    
    if (iTile >= 0):
        iOffset =  3*dim
        iStart = iOffset + npts_tile * iTile
    ...
    ncols = dim - 1
    nskip = 3
    lat_data = lat[ iStart : iEnd : nskip ]
    lon_data = lon[ iStart : iEnd : nskip ]
       
    if plot_data:
        var = df[varname]
        var_data = var[ 0, iStart : iEnd : nskip ]
        ( umin, umax ) = ( var_data.min(), var_data.max() )
        urange = ( umax - umin)
        ucscale = 255.0/urange
    ...
    max_cval = ( dim ) / 3 + 1
    cscale = 255.0/max_cval
    
    for iPt in range( 0, lat_data.shape[0] ):
        ix = iPt % ncols
        iy = iPt / ncols
        lx = lon_data[iPt]
        ly = lat_data[iPt]
        ...
        if plot_data:
            dval = var_data[ iPt ]
            Colors.InsertNextTuple3(  ( umax - dval ) * ucscale, 0, ( dval - umin ) * ucscale )
        else:
            if ( iPt % dim ) == 0:
                Colors.InsertNextTuple3(  0, 0, 0  )
            else:
                iC0 = iPt % max_cval
                c = ( ( max_cval - iC0 ) * cscale, 0,  iC0 * cscale  )
...
def test_image_data( data_file, varname, iTile  ): 
    import vtk, cdms2, time, random, math
    nTiles = 6
    df = cdms2.open( data_file )
    if df.source == 'CAM' and ( df.axes.get( "ncol", None ) <> None ):
        var = df[ varname ]
        ndims = len( var.shape )
        if ndims == 3:
...
#     outputExtent = [ 0, dim-1, 0, dim-1, 0, nlev-1 ]
#     gridSpacing = [ 1, 1, 1 ]
#     image_data.SetOrigin( outputOrigin[0], outputOrigin[1], outputOrigin[2] )
#     image_data.SetExtent( outputExtent )
#     image_data.SetWholeExtent( outputExtent )
...
#     vtkdata.SetVoidArray( var_data, var_data.size, 1 )
...
#     volumeMapper.SetInputConnection(image_data.GetProducerPort())
...
#     compositeOpacity.AddPoint(500000.0,1.0)
...
#    varname = "U"
    varname = "PS"
    tile_index = 3
#    test_image_data( CAM_file, varname, tile_index )
    test_points( CAM_file, grid_file, varname, tile_index )
#    main(sys.argv)
