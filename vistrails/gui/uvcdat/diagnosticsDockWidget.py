...
#      self.comboBoxVar.addItems(['Variables'])
      ...
      # that's basically all we can/should do until someone selects some directories, etc
		...
		# temporarily disable some widgets
		...
		self.DS1FilterEdit.setEnabled(False)
		self.DS1FilterLabel.setEnabled(False)
		...
		self.DS2FilterLabel.setEnabled(False)
...
#        print 'options.opts at apply clicked: ', self.opts._opts
   ...
   def itemClicked(self, item, column):
      print 'itemClicked called --- THIS SHOULDNT BE CALLED'
      quit()
      if column == 0:
         if item.checkState(column) == Qt.Checked:
            self.diagnostic_set_name = str(item.text(column))
