...
#                                                                             #
# Description:  UV-CDAT GUI plot view.                                        #
#                                                                             #
# Version:      6.0                                                           #
#                                                                             #
###############################################################################
...
class QPlotOptionsWidget(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.plotTypeCombo.addItems(customizeUVCDAT.plotTypes)
        self.plotTypeCombo.addItems(customizeUVCDAT.extraPlotTypes.keys())

        hbox.addWidget(self.plotTypeCombo)

        if uvcdatCommons.useVistrails:
            # Add cell row / col combo boxes
            sheetLabel = QtGui.QLabel('Sheet')
            rowLabel = QtGui.QLabel('Row')
            colLabel = QtGui.QLabel('Col')
            self.cellSheetCombo = QtGui.QComboBox()
            ...
            self.cellSheetCombo.addItem('Auto')
            self.cellRowCombo.addItem('Auto')
            self.cellColCombo.addItem('Auto')

            for i in range(1, 9):
                self.cellSheetCombo.addItems(str(i))
                self.cellRowCombo.addItems(str(i))
                self.cellColCombo.addItems(str(i))

            hbox.addWidget(sheetLabel)
            hbox.addWidget(self.cellSheetCombo)
    ...
    def selectedPlotType(self,*args):
        # setting parent to None so it is not deleted with QVCSPlotController
        self.parent.plotsSetup.setParent(None)
    ...
    def getSheet(self):
        if self.cellSheetCombo.currentText() == 'Auto':
            return -1
        return int(self.cellSheetCombo.currentText()) - 1
    ...
    def setPlotTypeComboItemEnabled(self, name, enable):
        index = self.plotTypeCombo.findText(QtCore.QString(name))
        item = self.plotTypeCombo.model().item(index,0)
...
class QPlotView(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for gm in [self.root.canvas[0].createboxfill,
            ...
            self.root.canvas[0].createxvsy,
            self.root.canvas[0].createscatter,
            self.root.canvas[0].createtaylordiagram,
            ]:
            try:
                gm(customizeUVCDAT.defaultGraphicsMethodName)
            except:
                pass
        try:
            self.root.canvas[0].createtemplate(customizeUVCDAT.defaultTemplateName)
        except:
            pass
        # Init layout
...
#        self.plotsSetup.setLayout(vlayout)
    ...
    def isRegisteredPlot(self, plotType):
        return customizeUVCDAT.extraPlotTypes.has_key(str(plotType))
...
#                        
                    ...
                    aspect = self.root.preferences.aspectType.buttonGroup.checkedId()
                    if aspect==-3:
                        plot_kargs["ratio"]="autot"
                    elif aspect==-4:
                        ratio = str(self.root.preferences.aspectCustom.text())
                        try:
                            ratio = float(ratio)
                            plot_kargs["ratio"]="%st" % ratio
                        except:
                            pass
...
#                        self.root.canvas[icanvas].plot(*vars,**plot_kargs)
            ...
            if isinstance(self.plotOptions.customizePlotFrame,
                ...
                sheet_name = self.plotOptions.customizePlotFrame.plot.cells[0].sheet_name
                ...
                self.plotOptions.customizePlotFrame.plot.alias_values[sheet_name] = self.plotOptions.getSheet()
    ...
    def nSlabsRequired(self,plotType):
        return uvcdatCommons.gmInfos.get(plotType,{}).get('nSlabs',-1)
    ...
    def updateOtherPlots(self, namelist):
        #print "updateOtherPlots ", namelist
        if len(namelist) == 0:
            #disable all plots
            for pname in customizeUVCDAT.extraPlotTypes.iterkeys():
                self.plotOptions.setPlotTypeComboItemEnabled(pname,False)
                
        for name in namelist:
            for pname, plot in customizeUVCDAT.extraPlotTypes.iteritems():
                if name not in plot.filetypes.keys():
                    self.plotOptions.setPlotTypeComboItemEnabled(pname,False)
                else:
                    self.plotOptions.setPlotTypeComboItemEnabled(pname, True)
