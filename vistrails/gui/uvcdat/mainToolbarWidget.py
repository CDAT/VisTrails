...
#                                                                             #
# Description:  UV-CDAT GUI main toolbar.                                     #
#                                                                             #
# Version:      6.0                                                           #
#                                                                             #
###############################################################################
...
class QMainToolBarContainer( QtGui.QWidget ):
    """ Main icon tool bar widget that is located at the top of UVCDAT's main
    window. """
    ...
    def __init__(self, parent=None):
        ...
        actionInfo = [
            ...
            ('esgf.png', 'Connection to the Earth System Grid Federation (ESGF) data archive.',self.ESGF,True),
            ('symbol_help.ico', 'Display assistant content for this application.',self.help,False),
            ]

        for info in actionInfo:
            pth = os.path.join(customizeUVCDAT.ICONPATH, info[0])
        ...
        if osprinters is None:
            ...
            printers=[osprinters,]
        vcsprinters =  sorted(uvcdatCommons.getAvailablePrinters())
        ...
        l = QtGui.QVBoxLayout()
        d.setLayout(l)
        fp = uvcdatCommons.QFramedWidget("Printer Selection",parent=self)
        ...
        self.esgf = esgf.QEsgfBrowser(parent=self.root)
        self.esgf.addGateway(gateway=customizeUVCDAT.defaultEsgfNode,mapping=customizeUVCDAT.defaultEsgfMapping)
        self.esgf.hide()

    def openScript(self):
        fnm = str(QtGui.QFileDialog.getOpenFileName(self,"Open Python Script",filter="Python Scripts (*.py);; All (*.*)"))
        d = uvcdatCommons.QCommandsFileWidget(self,fnm,False,fnm)
        d.show()
        self.files.append(d)
    ...
    def savePlots(self):
        for x in self.root.canvas:
            if x.iscanvasdisplayed():
                fnm = str(QtGui.QFileDialog.getSaveFileName(self,"Save Plot",filter="VCS Images (*.png *.svg *.gif *.pdf *.ps *.eps);; All (*.*)"))
                if fnm.lower()[-3:] == ".ps":
                    x.postscript(fnm)
