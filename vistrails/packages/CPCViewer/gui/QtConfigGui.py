...
from packages.CPCViewer.ColorMapManager import ColorMapManager
from packages.CPCViewer.gui.ControlPanel import *
from packages.CPCViewer.ConfigFunctions import ConfigManager, SIGNAL
from packages.CPCViewer.gui.ROISelection import ROIControl
...
def createConfigDialog( self, configCmdCallback=None, show=False, interface ):     
    configDialog = CPCConfigGui( self.point_cloud_overview.getPointCollection() )
    self.activateConfigDialog( self.configDialog, show )       
    w = configDialog.getConfigWidget()
    if configCmdCallback: w.ConfigCmd.connect( configCmdCallback )
    #    dialog.connect( self, QtCore.SIGNAL("UpdateGui"), dialog.externalUpdate )
    configDialog.activate()        
    if show: configDialog.show()
