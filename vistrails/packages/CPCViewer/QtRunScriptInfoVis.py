...
if ns.data_type == "GEOS5":   
    g = VolumePlot(gui=True) 
    g.init( init_args = ( grid_file, data_file, interface, varname, grid_coords, var_proc_op, roi, 'xyz' ), show=showGui ) 
else:
    g = CPCPlot(gui=True) 
    ncores=multiprocessing.cpu_count()
    g.init( init_args = ( grid_file, data_file, interface, varname, grid_coords, var_proc_op, roi, 'xyz' ), n_overview_points=n_overview_points, n_cores = ncores, show=showGui  )   # n_cores = ncores      
