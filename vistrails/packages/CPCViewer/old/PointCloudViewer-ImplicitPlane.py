...
#             print "Toggle Slider Interaction: %d" % (enabled); sys.stdout.flush()
    ...
    def enableSlicing( self ):
        ...
        if self.topo == PlotType.Planar: 
            self.initSliderWidget( self.point_cloud_overview.getPolydata(), self.point_cloud_overview.getBounds() )   
#            self.initPlaneWidget( self.point_cloud_overview.getPolydata(), self.point_cloud_overview.getBounds() )                      
            self.sliderWidgetOn(  )
...
#                self.partitioned_point_cloud.refresh(True)     
    ...
    def getSliceWidth(self, res, slice_index = -1  ):
        sliceProperties = self.getConfigFunction( 'sliceProp' ).value   
        if slice_index == -1: slice_index = self.sliceAxisIndex
        if slice_index == 2: return self.zSliceWidth
        else:
            if res == ProcessMode.LowRes:  return sliceProperties[ SLICE_WIDTH_LR_COMP[ slice_index ] ]
            if res == ProcessMode.HighRes: return sliceProperties[ SLICE_WIDTH_HR_COMP[ slice_index ] ]
            
    def getSlicePosition(self, slice_index = -1 ):
        sliceProperties = self.getConfigFunction( 'sliceProp' ).value   
        if slice_index == -1: slice_index = self.sliceAxisIndex
        return sliceProperties[ POS_VECTOR_COMP[ slice_index ] ]

    def setSlicePosition(self, slice_pos, slice_index = -1 ):
        sliceProperties = self.getConfigFunction( 'sliceProp' ).value   
        if slice_index == -1: slice_index = self.sliceAxisIndex
        sliceProperties[ POS_VECTOR_COMP[ slice_index ] ] = slice_pos
