#!/Applications/UVCDAT.app/Contents/Library/Frameworks/Python.framework/Versions/2.7/bin/python
...
def main(argv):
    ...
    boundsdict = {}
    fileIndex = 0                     # Same as vardict for time bounds
...
#                print " ** %s ** " % axisname; sys.stdout.flush()
                axis = f[axisname]
        ...
        for varname in varnames:
            ...
            if splitOnTime or forecast:
                ...
                if vartime is not None:
                    ...
                    vartime_data = vartime.getValue()
                    if len( vartime_data ) == 0:
                        vartime_data = numpy.array( [ fileIndex ] )
                    starttime = vartime_data[0]
                    endtime = vartime_data[-1]
                    startindex = timeindex(vartime_data[0], vartime.units, referenceTime, referenceDelta, calendar)
                    endindex = timeindex(vartime_data[-1], vartime.units, referenceTime, referenceDelta, calendar)
                    if forecast:
...
if __name__ == '__main__':
    data_dir = '/Users/tpmaxwel/Data/MERRA_MONTHLY' 
    data_files = [ os.path.join(data_dir,f) for f in os.listdir(data_dir) if f.endswith(".nc")  ]

    args = [  'cdscan', '--time-linear', '0,1,months since 1979-01-01', '-i', '1', '-r', 'months since 1979-01-01', '-d', 'merra_monthly', '-x', '/tmp/merra_monthly.xml'  ] +  data_files
    main(args)
