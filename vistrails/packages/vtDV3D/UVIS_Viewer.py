...
from UVIS_DV3D.Application import DV3D
from UVIS_DV3D.MultiVarPointCollection import InterfaceType
...
class PM_UVIS_Viewer(PersistentVisualizationModule):
    """
        This module wraps the UVIS_Viewer package. 
    ...
    def activateEvent( self, caller, event ):
        ...
        if self.renwin <> None:
            if self.plotter == None:
                ...
                self.plotter.init( init = ( self.grid_file, self.data_file, interface, [ self.varname ], grid_coords, var_proc_op, roi, 'xyz' ), renwin=self.renwin, n_overview_points=self.n_overview_points, n_cores=ncores ) # , n_subproc_points=100000000 )
#                self.getConfigWidget()
#                DV3DPipelineHelper.denoteUVIS_Viewer( self.moduleID )
    ...
    def addConfigurableFunctions( self ):
        pass
#         if PM_UVIS_Viewer.PortSpecs == None:
#             config_widget = ConfigurationWidget()
#             config_widget.build()
#             PM_UVIS_Viewer.PortSpecs = config_widget.cfgManager.getPersistentParameterSpecs()
#         for port_spec in PM_UVIS_Viewer.PortSpecs:
#             name = port_spec[0]
...
class UVIS_Viewer(WorkflowModule):
    
    PersistentModuleClass = PM_UVIS_Viewer
    
    def __init__( self, **args ):
        WorkflowModule.__init__(self, **args) 
        print " "
                
class UVIS_ViewerConfigurationWidget(StandardModuleConfigurationWidget):
